...
const makeroom = function () {
    JOINSTACK.forEach(item => {
        ...
        if (!CHATBOXCACHE.some(_room => {
            if (parseInt(chatboxid, 10) !== _room.chatboxid)
                return;
            return true;
        })) {
    ...
    JOINSTACK.forEach(item => {
        ...
        databasesync.getmemberOfchatbox(userid, chatboxid)
            ...
            CHATBOXCACHE.some(room => {
                if (parseInt(chatboxid, 10) !== room.chatboxid)
                    return;
                switch (true) {
                    case (room.members.some(_mem => { if (_mem.userid === parseInt(userid, 10))
                        return true; })): return false; // members is in room
                }
...
const freecache = function () {
    CHATBOXCACHE.forEach(room => {
        room.members.forEach(mem => {
            if (mem.sendedmessage.length <= lenOfcache)
                return;
...
const cleanchatbox = function () {
    CHATBOXCACHE.forEach(room => {
        if (room.members.length !== 0)
            return;
        if (isforgive(room.datetimeOpen))
            return;
            ...
            for (var mem of room.members) {
                for (var msg of mem.sendedmessage) {
                    switch (true) {
                        ...
                        default: yield databasesync.save(mem.userid, msg);
                    }
                }
            }
        }
    ...
    return new Promise(function (resolve, reject) {
        CHATBOXCACHE.forEach(room => {
            if (parseInt(chatboxid, 10) !== room.chatboxid)
                return;
            room.members.forEach(mem => {
                if (mem.userid !== parseInt(userid, 10))
                    return;
                room.members.remove(mem);
...
const postmessagetochtbx = function (userid, messageData, datetimesend, conversationid) {
    return new Promise(function (resolve, reject) {
        ...
        databasesync.getConversation(conversationid)
            ...
            for (var room of CHATBOXCACHE) {
                if (conversation.chatboxid !== room.chatboxid)
                    continue;
                for (var mem of room.members) {
                    if (_userid !== mem.userid)
                        continue;
        ...
        return __awaiter(this, void 0, void 0, function* () {
            databasesync.getConversation(conversationid)
                ...
                CHATBOXCACHE.forEach(room => {
                    if (conversation.chatboxid !== room.chatboxid)
                        return; //next
                    if (!room.members.some(_mem => { if (_userid === _mem.userid) {
                        user = _mem;
                        return true;
                    } }))
                        return;
                    ...
                    room.members.forEach(mem => {
                        for (var msg of mem.sendedmessage) {
                            if (msg.datetimesend < user.datetimejoin)
                                continue;
                            if (msg.userreaded.some(reader => { if (_userid === reader)
                                return true; }))
                                continue;
                            result.push({
                                unhideUsermindid: msg.messageid,
    ...
    *************************************************/
    ...
    for (var room of CHATBOXCACHE) {
        for (var item of chatlist) {
            if (room.chatboxid !== parseInt(item, 10))
                continue;
